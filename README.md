...
The data ```DivScene.zip``` are released at [DivScene-DivTraj](https://huggingface.co/datasets/ZhaoweiWang/DivScene-DivTraj) dataset on HuggingFace Hub.
...
## DivTraj Data
Similarly, the episodes of shortest paths we sampled are at [DivScene-DivTraj](https://huggingface.co/datasets/ZhaoweiWang/DivScene-DivTraj) dataset.
...
**LICENSE ISSUE:** We also release our revision of [Pai-Megatron](https://github.com/alibaba/Pai-Megatron-Patch) and [Megatron-LM](https://github.com/NVIDIA/Megatron-LM). 
If you use those code, all licenses are subject to their original release.

## Inference
We conduct the inference using the model serving mode. We deploy trained LVLM on Linux servers. 
