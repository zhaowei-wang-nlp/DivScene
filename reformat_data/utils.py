...
def get_cur_diff(r, p_diff):
    ...
    return diff_direction, diff_len, diff_len_abs


def get_other_diff(r, p_diff, cur_action):
    # load other dir
    if r in {90, 270}:
        other_diff_len = p_diff[1]
        other_diff_direction = "north" if other_diff_len > 0 else "south"
    else:
        other_diff_len = p_diff[0]
    ...
    return other_r, other_diff_direction, other_diff_len, other_diff_len_abs

def add_cot(cur_action, reachable_position_set, p, r, p_diff, target_agent_pos,
            target_agent_r, idx, action_list, object_type):
    if cur_action == "MoveAhead":
        diff_direction, diff_len, diff_len_abs = get_cur_diff(r, p_diff)
        step1 = move_template[0].format(r, direction_text_mapping[r], diff_len, diff_len_abs, diff_direction, diff_direction)
        ...
        if idx == len(action_list) - 2:
            ...
            if r in {90, 270}:
                ...
                diff_len = p_diff[1]
            assert diff_len == 0
            other_r, other_diff_direction, other_diff_len, other_diff_len_abs = get_other_diff(r, p_diff, cur_action)
            step1 = half_end_rotate_template[0].format(r, direction_text_mapping[r], diff_len,
                                                       other_diff_len, other_diff_len_abs, other_diff_direction,
                                                       other_r)
            ...
            diff_direction, diff_len, diff_len_abs = get_cur_diff(r, p_diff)
            other_r, other_diff_direction, other_diff_len, other_diff_len_abs = get_other_diff(r, p_diff, cur_action)
            step1 = block_rotate_template[0].format(r, direction_text_mapping[r], diff_len)
            step2 = block_rotate_template[1].format(other_diff_len, other_diff_len_abs, other_diff_direction, other_r)
...
def get_llava_format_data(traj_dir, task_list, trial_num, sample_rate, image_num, action_his_num, use_cot):
    data_list = []
    ...
    for task_name in tqdm(task_list, f"checking actions in train split"):
        ...
        for idx, cur_action in enumerate(action_list):
            ...
            if use_cot:
                ...
                output_action = add_cot(cur_action, reachable_position_set, p, r, p_diff, target_agent_pos,
                                        target_agent_r, idx, action_list, object_type)
            else:
...
def get_llava_format_data_no_diff(traj_dir, task_list, trial_num, sample_rate, image_num, action_his_num, use_cot):
    data_list = []
    prev_image_num = image_num - 1
    for task_name in tqdm(task_list, f"checking actions in train split"):
        # check the trial number
...
def train_data_filter(data_list, traj_dir):
    # List all items in the given directory
    new_data_list, removed_dis = [], defaultdict(int)
    prompt_set = defaultdict(tuple)
    for d in tqdm(data_list, "checking data"):
        task_id, action_id = d["id"].split("_")
        scene_id, trial_id = task_id.split("-")
        scene_id, trial_id, action_id = int(scene_id), int(trial_id), int(action_id[1:])
        traj_file_path = os.path.join(traj_dir, str(task_id), "traj.json")
        with open(traj_file_path) as fin:
            traj_json = json.load(fin)
        action_list = traj_json["traj_info"]["action"]
        action_list = action_list[:action_id]
        action_count = Counter(action_list)
        ma_count, rr_count, rl_count = action_count["MoveAhead"], action_count["RotateRight"], action_count[
            "RotateLeft"]
        position_list = traj_json["traj_info"]["corner"]
        example_pos = position_list[ma_count]
        rotation = (traj_json["init_agent_status"]["rotation"]["y"] + (rr_count - rl_count) * 90 + 360) % 360
        example_pos = f"{example_pos[0]}_{example_pos[1]}_{rotation}"

        input_prompt = d["conversations"][0]["value"]
        object_type = extract_object_type(traj_json["objectId"])
        cur_example = (scene_id, example_pos, object_type,
                       input_prompt[input_prompt.index("The history of recent states are:\n"):].strip())

        # same scene + same position + same action history
        # should have same output
        if cur_example not in prompt_set:
            prompt_set[cur_example] = d["conversations"][1]["value"]
            new_data_list.append(d)
        elif d["conversations"][1]["value"] == prompt_set[cur_example]:
            new_data_list.append(d)
        else:
            removed_dis[d["conversations"][1]["value"]] += 1
            pass

    print(removed_dis)
    print(f"finished, {len(data_list)} deduplicate {len(new_data_list)}")
